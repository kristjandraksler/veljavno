      ...
      <div className="bg-primary">
        <div className="max-w-3xl mx-auto px-6 py-16 text-white">
          <div className="flex items-center gap-3 mb-6">
            <span className="text-4xl">{clanek.ikona}</span>
            <span className="text-sm font-semibold bg-white/20 px-3 py-1 rounded-full">{clanek.kategorija}</span>
          </div>
          <h1 className="font-display text-3xl font-bold tracking-[-0.04em] mb-4 md:text-4xl leading-tight">{clanek.naslov}</h1>
          <div className="flex items-center gap-4 text-blue-200 text-sm mt-6">
      ...
      <div className="max-w-3xl mx-auto px-6 py-16">
        <div
          className="prose prose-lg max-w-none text-foreground
            [&_h2]:text-2xl [&_h2]:font-bold [&_h2]:mt-12 [&_h2]:mb-4 [&_h2]:text-foreground [&_h2]:border-b [&_h2]:border-border [&_h2]:pb-3
            ...
            [&_ul]:list-none [&_ul]:pl-0 [&_ul]:mb-4
            [&_li]:text-muted-foreground [&_li]:mb-2 [&_li]:flex [&_li]:items-start [&_li]:gap-2
            [&_li]:before:content-['✓'] [&_li]:before:text-primary [&_li]:before:font-bold [&_li]:before:flex-shrink-0
        ...
        <div className="mt-16 bg-primary rounded-[2rem] p-10 text-white text-center">
          ...
          <h2 className="text-2xl font-bold mb-3">Ne pozabite na opomnik!</h2>
          <p className="text-blue-100 mb-6 max-w-md mx-auto">Veljavno vas avtomatsko opomni preden poteče vaš dokument — teden, mesec ali 6 mesecev prej.</p>
          <a href="/registracija" className="inline-block bg-white text-primary px-8 py-4 rounded-full text-sm font-semibold uppercase tracking-[0.16em] hover:bg-blue-50 transition-colors">
            Začni za 4,99 € →
          </a>
        </div>
        ...
        {/* Drugi članki */}
        <div className="mt-16">
          <h3 className="text-lg font-bold mb-6">Preberite tudi</h3>
          <div className="grid gap-4 md:grid-cols-2">
            {Object.entries(clanki)
              .filter(([s]) => s !== slug)
              .map(([s, c]) => (
                <a key={s} href={`/blog/${s}`} className="group bg-card border border-border rounded-2xl p-6 hover:border-primary transition-colors">
                  <div className="text-2xl mb-3">{c.ikona}</div>
                  <h4 className="font-semibold text-sm group-hover:text-primary transition-colors leading-snug">{c.naslov}</h4>
                  <p className="text-xs text-muted-foreground mt-2">{c.branje} branja</p>
                </a>
              ))}
          </div>
        </div>
